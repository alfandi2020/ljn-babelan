        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Table Cetak Pembayaran Pelanggan</h4>
                </div>
                        ...
                        <form action="<?= base_url('pelanggan/sort/bulan') ?>" method="POST">
                            <div class="row">
                                <!-- <div class="col-xl-3 col-sm-5 mt-2">
                                    ...
                                    <input type="number" required name="tgl_t" min="1" max="31"
                                        value="<?= $this->session->userdata('filterTgl_tempo') == false ? 10 : $this->session->userdata('filterTgl_tempo') ?>"
                                ...
                                <div class="col-xl-3 col-sm-5 mt-2">
                                    ...
                                    <fieldset>
                                        <select onchange="this.form.submit()" name="tahun_t" id="" class="form-control">
                                            <option <?= $this->session->userdata('filterTahun') == date('Y') - 1 ? 'selected' : '' ?> value="<?= date('Y') - 1 ?>">
                                                <?= date('Y') - 1 ?>
                                            </option>
                                            <option <?= $this->session->userdata('filterTahun') == date('Y') ? 'selected' : '' ?> value="<?= date('Y') ?>">
                                                <?= date('Y') ?>
                                            </option>
                                            <option <?= $this->session->userdata('filterTahun') == date('Y') + 1 ? 'selected' : '' ?> value="<?= date('Y') + 1 ?>">
                                                <?= date('Y') + 1 ?>
